...
const FILTERS = [
  { key: "all", label: "All", sub: "20 designs" },
...
export default function SocialMediaPostsPage() {
  ...
  const feedCount = SOCIAL_POSTS.filter((p) => p.format === "feed").length;
  const storyCount = SOCIAL_POSTS.filter((p) => p.format === "story").length;
  ...
  return (
    <main className="relative flex flex-1 flex-col">
      <PageHero
        ...
        intro="The full visual campaign — twenty editorial creatives across Instagram feed and stories. Every artboard renders live, exactly as designed."
      >
      ...
      <SectionFrame
        id="gallery"
        label="02 — Gallery"
        number={`${visible.length} of ${SOCIAL_POSTS.length}`}
      >
